...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.error) {
      setError('Email ose fjalekalimi nuk jane te sakta.');
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div>
            <h2 className="text-4xl font-bold text-white mb-4 leading-tight">Mire se vini<br />perseri.</h2>
            <p className="text-white/60 text-lg leading-relaxed max-w-md">
              Kycuni ne llogarine tuaj per te menaxhuar rezervimet, automjetet dhe me shume.
            </p>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 sm:px-10 py-12 bg-gray-50">
        <div className="w-full max-w-[420px]">
          ...
          <h1 className="text-2xl font-bold text-dark-950 mb-1.5">Kycu ne llogari</h1>
          <p className="text-dark-500 mb-8 text-[15px]">Vendosni kredencialet tuaja per te vazhduar</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-dark-700 mb-1.5">Fjalekalimi</label>
            ...
            >
              ...
              {loading ? 'Duke u kyqur...' : 'Kycu'}
            </button>
          </form>

          <p className="mt-8 text-center text-sm text-dark-500">
            Nuk keni llogari?{' '}
            <Link to="/regjistrohu" className="text-primary-600 font-semibold hover:text-primary-700 transition-colors">Regjistrohu</Link>
          </p>

          <div className="mt-8 pt-8 border-t border-gray-200">
            <p className="text-xs font-semibold text-dark-400 uppercase tracking-wider mb-3 text-center">Llogari Demo</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: 'Admin', email: 'admin@rentacar.com', password: 'Admin123!', icon: Shield, color: 'text-red-600', bg: 'bg-red-50 hover:bg-red-100 border-red-100' },
                { label: 'Kompania', email: 'company@rentacar.com', password: 'Company123!', icon: Building2, color: 'text-amber-600', bg: 'bg-amber-50 hover:bg-amber-100 border-amber-100' },
                { label: 'Klienti', email: 'client@rentacar.com', password: 'Client123!', icon: User, color: 'text-primary-600', bg: 'bg-primary-50 hover:bg-primary-100 border-primary-100' },
              ].map((acc) => (
                <button
                  key={acc.email}
                  type="button"
