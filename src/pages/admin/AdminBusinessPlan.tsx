...
export default function AdminBusinessPlan() {
  ...
  async function loadData() {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({
        month: date.toLocaleDateString(dateLocale, { month: 'short', year: 'numeric' }),
  ...
  return (
    <DashboardLayout navItems={adminNavItems} navGroups={adminNavGroups} title={t('adminDash.businessPlan.pageTitle')}>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('adminDash.businessPlan.pageTitle')}</h1>
            <p className="text-gray-500 text-sm mt-1">{t('adminDash.businessPlan.subtitle')}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={loadData} className="flex items-center gap-2 border border-gray-200 text-gray-600 hover:bg-gray-50 px-4 py-2 rounded-lg text-sm font-medium">
              <RefreshCw className="w-4 h-4" />{t('adminDash.businessPlan.refresh')}
            </button>
            <button onClick={() => exportToCSV(monthlyData as unknown as Record<string, unknown>[], 'business_plan')}
              ...
              <Download className="w-4 h-4" />{t('adminDash.businessPlan.export')}
            </button>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2 grid grid-cols-3 gap-4">
                {[
                  { label: t('adminDash.businessPlan.statRevenue'), value: `€${totals.revenue.toLocaleString()}`, icon: DollarSign, color: 'green' },
                  { label: t('adminDash.businessPlan.statBookings'), value: totals.bookings.toLocaleString(), icon: Calendar, color: 'blue' },
                  { label: t('adminDash.businessPlan.statCompanies'), value: totals.companies, icon: Building2, color: 'teal' },
                  { label: t('adminDash.businessPlan.statUsers'), value: totals.users, icon: Users, color: 'orange' },
                  { label: t('adminDash.businessPlan.statVehicles'), value: totals.vehicles, icon: Car, color: 'gray' },
                  { label: t('adminDash.businessPlan.statSubRevenue'), value: `€${totals.subscriptionRevenue.toLocaleString()}`, icon: CreditCard, color: 'purple' },
              ...
              <div className="bg-gradient-to-br from-primary-600 to-purple-700 rounded-xl p-6 text-white">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="text-xs text-primary-200">{t('adminDash.businessPlan.monthlyEst')}</div>
                    <div className="text-3xl font-black mt-1">€{projections.monthly.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-xs text-primary-200">{t('adminDash.businessPlan.yearlyProjection')}</div>
                  ...
                  <div className={`flex items-center gap-2 ${projections.growth >= 0 ? 'text-green-300' : 'text-red-300'}`}>
                    ...
                    <span className="text-sm font-medium">{t('adminDash.businessPlan.growthVsLastMonth', { prefix: projections.growth >= 0 ? '+' : '', value: projections.growth })}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              <h3 className="font-semibold text-gray-900 mb-6 flex items-center gap-2"><BarChart2 className="w-5 h-5 text-primary-600" />{t('adminDash.businessPlan.revenueByMonth')}</h3>
            ...
            <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                <h3 className="font-semibold text-gray-900">{t('adminDash.businessPlan.monthlyDetails')}</h3>
              </div>
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    {[t('adminDash.businessPlan.thMonth'), t('adminDash.businessPlan.thRevenue'), t('adminDash.businessPlan.thBookings'), t('adminDash.businessPlan.thNewCompanies'), t('adminDash.businessPlan.thNewUsers')].map((h, idx) => (
                      <th key={idx} className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
