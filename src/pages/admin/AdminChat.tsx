...
const categories = [
  ...
  { value: 'pergjithshme', label: 'Te pergjithshme' },
  { value: 'rezervime', label: 'Rezervime' },
  { value: 'automjete', label: 'Automjete' },
  { value: 'cmime', label: 'Cmime' },
  { value: 'pagesa', label: 'Pagesa' },
  { value: 'llogari', label: 'Llogari' },
  { value: 'kompani', label: 'Kompani' },
  { value: 'sigurime', label: 'Sigurime' },
  { value: 'dokumente', label: 'Dokumente' },
  { value: 'lokacione', label: 'Lokacione' },
  { value: 'suport', label: 'Suport' },
  { value: 'kushte', label: 'Kushte' },
  { value: 'teknike', label: 'Teknike' },
  { value: 'ankesa', label: 'Ankesa' },
];
...
export default function AdminChat() {
  ...
  async function deleteResponse(id: string) {
    if (!confirm('Fshij kete pergjigje?')) return;
  ...
  return (
    <DashboardLayout title="Admin" navItems={adminNavItems} navGroups={adminNavGroups}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-dark-950">Menaxhimi i Chat AI</h1>
          <p className="text-dark-500 mt-1 text-[15px]">{total} pergjigje automatike</p>
        </div>
      ...
      {(creating || editing) && (
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs font-medium text-dark-600 mb-1.5">Kategoria</label>
              <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} className={inputClass}>
                {categories.filter(c => c.value).map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-dark-600 mb-1.5">Prioriteti (me i larte = me e rendesishme)</label>
              <input type="number" value={form.priority} onChange={e => setForm(f => ({ ...f, priority: +e.target.value }))} className={inputClass} />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-dark-600 mb-1.5">Pyetja</label>
              <input value={form.question} onChange={e => setForm(f => ({ ...f, question: e.target.value }))} className={inputClass} placeholder="p.sh. Si mund te bej nje rezervim?" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-dark-600 mb-1.5">Pergjigja</label>
              <textarea value={form.answer} onChange={e => setForm(f => ({ ...f, answer: e.target.value }))} rows={3} className={`${inputClass} resize-none`} placeholder="Pergjigja e detajuar..." />
            </div>
          </div>
          <div className="mb-4">
            <label className="block text-xs font-medium text-dark-600 mb-1.5">Fjalet kyqe (per matching)</label>
            <div className="flex gap-2 mb-2">
              <input value={keywordInput} onChange={e => setKeywordInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addKeyword())} className={`${inputClass} flex-1`} placeholder="Shto fjale kyqe..." />
              <button onClick={addKeyword} className="px-4 py-2 bg-gray-100 text-dark-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors">Shto</button>
            </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all" placeholder="Kerko pyetje ose pergjigje..." />
          </div>
          ...
          <>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-[10px] text-dark-300">Prioriteti: {r.priority}</span>
                        {!r.is_active && <span className="text-[10px] text-red-500 font-semibold">JOAKTIVE</span>}
                        <span className="text-[10px] text-dark-300 ml-auto">Perdorur {r.usage_count}x</span>
                      </div>
            ...
            {totalPages > 1 && (
              <div className="px-5 py-3 border-t border-gray-100 flex items-center justify-between">
                <p className="text-xs text-dark-400">Faqja {page + 1} nga {totalPages} ({total} gjithsej)</p>
                <div className="flex gap-1.5">
                  <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="px-3 py-1.5 text-xs font-medium bg-gray-50 rounded-lg hover:bg-gray-100 disabled:opacity-40 transition-colors">Para</button>
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="px-3 py-1.5 text-xs font-medium bg-gray-50 rounded-lg hover:bg-gray-100 disabled:opacity-40 transition-colors">Tjetra</button>
                </div>
              </div>
            )}
          </>
